...
class ParseLog {
  ...
  async start(date) {
    ...
    let logSize = 0;
    logs.forEach((log) => {
      logSize += fs.statSync(`${this.originalLogsDir}/${log}`).size;
    });

    console.log(`log大小: ${logSize / 1024 / 1024} MB`);
  }
      ...
      if (currentError) {
        ...
        if (
          ...
          (i < errorLines.length - 1 &&
            errorLines[i + 1].includes("Error Details:"))
        ) {
  ...
  processErrorBlock(error, errorLines) {
    // 只保留包含 axios 相关信息的错误
    const hasAxiosContent = errorLines.some(
      (line) =>
        line.includes("AxiosError:") ||
        line.includes("axios") ||
  ...
  generateErrorAnalysis(errors, date, savePath, analysisFileName) {
    ...
    const networkErrors = errors.filter(
      (error) =>
        ...
        (error.details.url.startsWith("http://") ||
          error.details.url.startsWith("https://"))
    );
