...
class ParseLog {
  ...
  async start(date) {
    ...
    const logs1 = fs
      .readdirSync("logs/api1")
      .filter((log) => log.includes(this.serviceName) && log.includes(date));

    const logs2 = fs
      .readdirSync("logs/api2")
      .filter((log) => log.includes(this.serviceName) && log.includes(date));

    const logs = [
      ...logs1.map((log) => `api1/${log}`),
      ...logs2.map((log) => `api2/${log}`),
    ];
  ...
  parseDevice(logs, date) {
    ...
    // 处理每个日志文件
    logs.forEach((log) => {
      const content = fs.readFileSync(`logs/${log}`, "utf-8");
  ...
  parseLogDetail() {
    ...
    files.forEach((file) => {
      ...
      const content = fs.readFileSync(`data/logs/${file}`, "utf-8");
      const data = JSON.parse(content);
